            ...
            for att in attempts:
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <button class='view-btn'
                            onclick="const outDiv=this.nextElementSibling;
                                     outDiv.style.display = (outDiv.style.display === 'block') ? 'none' : 'block';">
                            Output
                        </button>
                        <div style='margin-top:4px;max-width:250px;font-family:monospace;font-size:0.88em;white-space:pre-wrap;background:#f7f7f7;border-radius:6px;padding:6px;color:#444;border:1px solid #e0e0e0; display:none;'>
                            <strong>Preview:</strong><br>
...
"""
        },
        {
            "question": (
                ...
                "'from_date': 3 days back date ,(DD-MM-YYYY)\n"
                " 'to_date': today,(DD-MM-YYYY)\n"
...
def send_hr_email(user_id):
    ...
    try:
        resend.api_key = "re_mPbaRVm3_LULoodCqDGJAm8PLCizccdE8"#"re_i3U1rY2s_4PzP6ZZFC89Wxqtt9Sizs9xy"
      ...
      <body>
        <div class="instructions-panel">
          ...
          <ul class="instructions-list">
            ...
            <li>You may use the 'Run Code/Save Code' button to check your code output before final submission.</li>
...
def render_question(user_id, question_idx, last_code="", dhan_client_id=None, dhan_access_token=None, time_left=None):
    ...
    if last_code:
        ...
        if dhan_client_id or dhan_access_token:
            code_default = (
                f'# Print Your API credentials Initially: or Print Attending \n'
    ...
    (function () {
        ...
        // Instead of listening for DOMContentLoaded, immediately run clearViolationForClick for all relevant IDs (be robust: check button exists before binding)
        ['submitForm', 'skipForm', 'runCodeBtn'].forEach(function (id) {
            clearViolationForClick([id]);
        });
    ...
    submit_button_text = "Save Code and Press Next →" if not is_last_question else "Submit Exam"

    submit_button_html = (
        f"<button id='submitBtn' type='submit' action='/submit' style='font-size:1.005em;"
...
async def submit(
    ...
    try:
        ...
        # Always inform HR on submit - regardless of question index
        send_hr_email(uid)
